...
const ProductCard: React.FC<{ p: Product; onOpen: (p: Product) => void }> = ({ p, onOpen }) => {
  ...
  return (
    <div className="h-full flex flex-col rounded-3xl border border-gray-100 bg-white p-4 shadow-sm hover:shadow-md hover:-translate-y-[1px] transition">
      {/* Image */}
      <div
      ...
      >
        ...
        <div className="absolute right-3 top-3 z-10 flex flex-col gap-2">
          <button
            className="grid place-items-center h-9 w-9 rounded-full bg-white/90 border border-black/10 shadow"
      ...
      <div className="mt-4 flex-1 flex flex-col">
        ...
        <h3
          className="mt-0.5 text-[15px] md:text-base font-semibold leading-snug line-clamp-2 hover:underline cursor-pointer"
          onClick={() => onOpen(p)}
        >
          {p.name}
        </h3>
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="text-2xl font-extrabold">{formatINR(p.price)}</div>
...
/* ================= Build from Static JSON (Samyang) ================= */
...
// Filter brand "Samyang" (case-insensitive), map to local Product shape
const items: Product[] = ALL_STATIC
  .filter((p) => (p.brand ?? "").toLowerCase() === "samyang" || (p.brand ?? "") === " ")
  .map((p) => ({
    id: p.id,
    slug: p.slug,
    name: p.name,
    description: p.alt,         // optional short copy
    image: p.image,
    alt: p.alt ?? p.name,
    brand: p.brand,
    price: p.price,
...
export default function LifeProducts() {
  ...
  const openStatic = (p: Product) => navigate(`/product-s/${p.slug ?? p.id}`);

  return (
    <div className="container mx-auto px-4 py-10">
      <header className="mb-2">
        <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">Samyang Products</h2>
        ...
        <div className="grid grid-cols-2 gap-4 items-stretch content-start">
          {items.map((p) => (
            <div key={String(p.id)} className="h-full">
              <ProductCard p={p} onOpen={openStatic} />
            </div>
          ))}
        </div>
      </div>
